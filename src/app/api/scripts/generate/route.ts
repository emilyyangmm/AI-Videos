...
**绝对指令**：
- 严格字数：你的 JSON 中 "script" 字段的总字数【必须】在 ${wordCount.min} 到 ${wordCount.max} 之间。
- 如果超过 ${wordCount.max} 个字，我的系统会直接崩溃，请务必精简口播语言，去掉废话！
- ${videoDuration}秒的时长非常短，请直接说重点，不要任何前缀。
- 开头第一句必须使用钩子词根
- 只输出纯口播文案，不需要分镜描述
- 结尾必须有明确的行动号召
...
请基于以上选题，生成口播脚本。注意：字数严格控制在 ${wordCount.min}-${wordCount.max} 字以内！`
      : `请根据行业"${userIndustry}"和视频目的"${videoGoal}"，结合选中的爆款元素和钩子词根，生成一个口播脚本。字数要求：${wordCount.min}-${wordCount.max} 字。`;
    ...
    // 后端暴力裁剪：如果 AI 实在太话痨，超标太多，强制截断
    if (finalScript.length > wordCount.max + 20) {
      console.warn(`字数超标严重: ${finalScript.length} > ${wordCount.max}，强制截断`);
      finalScript = finalScript.substring(0, wordCount.max);
    }
